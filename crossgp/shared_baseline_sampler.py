...
class SharedBaselineSampler:
    ...
    def __init__(self, data_nights, kerns, noise_nights, prior_bounds, umin, umax, uv_bins_du, k_eor=None, bl_noise=True):
        ...
        self.rho_map = self.compute_rho_map()
    ...
    def compute_rho_map(self):
        rho_map = []
        for idx, name in enumerate(self.param_names_flat):
            if not name.endswith('.rho'):
                continue
            coh_comp = name[:-4]  # strip '.rho'
            parts = coh_comp.split('.')
            inc_comp = parts[0] + '_inc' + ('.' + '.'.join(parts[1:]) if len(parts) > 1 else '')
            var_name = coh_comp + '.variance'
            if var_name not in self.param_names_flat:
                raise ValueError(f"rho param '{name}' requires '{var_name}' to also be in the coherent dict")
            var_idx = self.param_names_flat.index(var_name)
            rho_map.append((idx, var_idx, coh_comp, inc_comp))
        return rho_map
    ...
    def set_params_bl(self, thetas, kerns):
        ...
        for rho_idx, var_idx, coh_comp, inc_comp in self.rho_map:
            rho = float(thetas[rho_idx])
            total_variance = float(thetas[var_idx])
            first_segment = inc_comp.split('.')[0]
            inc_has_subcomp = hasattr(kerns[0][1], first_segment)
            for k in range(self.N_bl):
                coh_kern_comp = traverse(kerns[k][0], coh_comp)
                inc_kern_comp = traverse(kerns[k][1], inc_comp) if inc_has_subcomp else kerns[k][1]
                coh_kern_comp.variance = rho * total_variance
                inc_kern_comp.lengthscale = coh_kern_comp.lengthscale
                inc_kern_comp.variance = (1 - rho) * total_variance
        return kerns
